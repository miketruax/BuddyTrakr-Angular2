...
export class BuddyService {
  ...
  loadBuddies() {
    ...
    this.http.get("/api/buddy", { headers: headers })
      .pipe(
        map(payload => {
          return payload["err"] ? 
            { type: flashActions.ADD_ERROR, payload: payload["err"] } 
          : { type: buddyActions.ADD_BUDDIES, payload: payload["buddy"] };
        }))
  ...
  updateBuddy(buddy: Buddy, bool: boolean = false) {
    let msg: string;
    if (bool) {
      msg = `${buddy.name} successfully checked ${
        buddy.checkedOut ? "out" : "in"
      }.`;
    } else {
